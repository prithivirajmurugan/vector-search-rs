...
impl<const N:usize> Vector<N>{
    pub fn substract_from(&self,vector:&Vector<N>)->Vector<N>{
...
impl<const N:usize> ANNIndex<N>{
    fn build_hyperplane(indexes:&Vec<usize>,all_vecs:&Vec<Vector<N>>)->(HyperPlane<N>,Vec<usize>,Vec<usize>){
        let sample:Vec<_> = indexes.choose_multiple(&mut rand::thread_rng(), 2).collect();
        // cartersian eq for hyperplane n*(x-x_0) = 0
        // where n (normal vector) is the coefs x_1 to x_n
        let (a,b) = (*sample[0],*sample[1]);
        let coefficients = all_vecs[a].substract_from(&all_vecs[b]);
    ...
    fn deduplicate(vectors:&Vec<Vector<N>>,ids:&Vec<i32>,dedup_vectors:&mut Vec<Vector<N>>,ids_of_dedup_vectors:&mut Vec<i32>){
    ...
    pub fn build_index(num_trees:i32,max_size:i32,vecs:&Vec<Vector<N>>,vec_ids:&Vec<i32>) -> ANNIndex<N>{
        ...
        let trees:Vec<_> = (0..num_trees).into_par_iter().map(|_| Self::build_a_tree(max_size, &all_indexes, &unique_vecs));
        return ANNIndex::<N>{
            trees,
            ids,
            values:unique_vecs,
    ...
    fn tree_result(query:Vector<N>,n:i32,tree:&Node<N>,candidates:&DashSet<usize>)->i32{
        match tree{
            ...
            Node::Inner(inner)=>{
                ...
                match Self::tree_result(query, n, tree, candidates) {
                    k if k < n => {
